...
export default class Member extends AggregateRoot {
  ...
  get minecraftUuid(): string | undefined {
    return this._minecraftUuid;
  }
  get minecraftName(): string | undefined {
    return this._minecraftName;
  }
  get guildId(): string | undefined {
    return this._guildId;
  }
  get guildJoinedAt(): Date | undefined {
    return this._guildJoinedAt;
  }

  static createNew(discordId: string, discordName: string): Member {
    return new Member(undefined, discordId, discordName);
  }
  ...
  leaveGuild(): void {
    ...
    if (!this._guildJoinedAt) {
      this.log.warn(
        `Member ${this.discordId} (${this.discordName}) has no guildJoinedAt date but is in guild ${this._guildId}. Setting guildJoinedAt to now.`,
      );
      this._guildJoinedAt = new Date();
    }
