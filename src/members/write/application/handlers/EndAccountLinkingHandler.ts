...
import {
  ...
  LinkingCodeRepositoryToken,
} from '../ports/LinkingCodeRepository';
import {
  type MemberRepository,
  MemberRepositoryToken,
} from '../ports/MemberRepository';
...
export class EndAccountLinkingHandler implements ICommandHandler<EndAccountLinkingCommand> {
  ...
  constructor(
    ...
    @Inject(LinkingCodeRepositoryToken)
  ...
  async execute({
    ...
    minecraftUuid,
    minecraftName,
  }: EndAccountLinkingCommand): Promise<void> {
    const discordId =
      await this.linkingCodeRepository.findDiscordIdByCode(linkingCode);

    if (!discordId) {
      throw new InvalidLinkingCode(linkingCode);
    }

    const existingMemberWithMinecraft =
      await this.memberRepository.findByMinecraftUuid(minecraftUuid);
    if (existingMemberWithMinecraft) {
      this.logger.warn(
        `INCONSISTENT DATA!!!: Attempt to link Minecraft account UUID ${minecraftUuid} (${minecraftName}) which is already linked to Discord ID ${existingMemberWithMinecraft.discordId}`,
      );
      throw new MinecraftAccountAlreadyLinkedError(
        discordId,
        existingMemberWithMinecraft.discordId,
      );
    }
    ...
    if (!member) {
      this.logger.error(
        `INCONSISTENT DATA!!!: Member not found for Discord ID: ${discordId}. This should not happen as we verified the linking code exists.`,
      );
