...
export class StartAccountLinkingHandler implements ICommandHandler<StartAccountLinkingCommand> {
  ...
  async execute({
    discordId,
    discordName,
  }: StartAccountLinkingCommand): Promise<string> {
    ...
    const existingLinkingCode =
      await this.linkingCodeRepository.findCodeByDiscordId(discordId);
    if (existingLinkingCode) {
      return existingLinkingCode;
    }

    const linkingCode =
      await this.linkingCodeRepository.generateAndSaveCode(discordId);
    member.commit();
